...
def wendetangente(flowrate, feed, ind_sprung, feed_max, feed_min, flowrate_max, flowrate_min, ind_wp, steigung_wp, step_dir):
    try:
        ...
        # Convert data to numpy arrays for proper indexing
        flow_data = flowrate['interp_y_smooth'].values
        time_data = np.array([t for t in flowrate['interp_dt']])
        feed_data = feed['interp_diff_substrate_feed'].values
        
        # Ensure indices are within bounds
        ind_sprung = min(int(ind_sprung), len(flow_data)-1)
        ind_wp = min(int(ind_wp), len(flow_data)-1)
        
        wt['K'] = (flowrate_max - flowrate_min)/(feed_max - feed_min)
        wt['ind_sprung'] = ind_sprung
        wt['zeit_sprung'] = time_data[ind_sprung]
        
        wt['index'] = ind_wp
        wt['steigung'] = steigung_wp
        wt['wert'] = flow_data[ind_wp]
        wt['offset'] = wt['wert'] - wt['steigung'] * ind_wp
        wt['t'] = np.arange(len(time_data))
        wt['gleichung'] = wt['steigung'] * wt['t'] + wt['offset']
        
        # Find Tu and Tg with safety checks
        if 'down' in step_dir:
        ...
        if len(tu_indices) == 0 or len(tg_indices) == 0:
            print("Warning: Using fallback indices for Tu/Tg")
            wt['ind_Tu'] = ind_sprung + 1
            wt['ind_Tg'] = ind_wp
        else:
            wt['ind_Tu'] = tu_indices[-1]
            wt['ind_Tg'] = tg_indices[0]

        # Calculate time differences in seconds
        def time_diff_seconds(t1, t2):
            return (pd.Timestamp(t1) - pd.Timestamp(t2)).total_seconds()
        
        wt['Tu'] = time_diff_seconds(time_data[wt['ind_Tu']], time_data[ind_sprung])
        wt['Tg'] = time_diff_seconds(time_data[wt['ind_Tg']], time_data[wt['ind_Tu']])
        wt['Twp'] = time_diff_seconds(time_data[ind_wp], time_data[ind_sprung])
        ...
        wt['Ta'] = 120
        wt['test_time'] = np.arange(0, wt['Ta'] * len(time_data), wt['Ta'])

        def create_tf_and_simulate(T):
            num = [wt['K']]
            den = np.convolve([T, 1], [T, 1])
            sys_d = signal.cont2discrete((num, den), wt['Ta'], method='zoh')
            u = feed_data - (feed_max if 'down' in step_dir else feed_min)
            _, yd = signal.dlsim(sys_d, u)
            offset = flowrate_max if 'down' in step_dir else flowrate_min
            return yd.flatten() + offset
        
        # Calculate all responses
        wt['yd_Twp'] = create_tf_and_simulate(wt['T_Twp'])
        wt['yd_Tu'] = create_tf_and_simulate(wt['T_Tu'])
        wt['yd_Tg'] = create_tf_and_simulate(wt['T_Tg'])
        wt['yd_Tmean'] = create_tf_and_simulate(wt['T_mean'])
        wt['yd'] = wt['yd_Tg']
        
        return wt
        
    except Exception as e:
        print(f"Error in wendetangente: {e}")
        traceback.print_exc()
        return None
...
def plot_wendetangente_ui(i, ui_axes, flowrate, feed, wt):
    ...
    ui_axes.set_xlabel('Time', 
                      fontname=const['font'], 
                      fontsize=const['fontsize'])
    ui_axes.set_title('Turning tangent method', 
                     fontname=const['font'], 
                     fontsize=const['fontsize'])
    ...
    for label in ui_axes.get_xticklabels() + ui_axes.get_yticklabels():
        label.set_fontname(const['font'])
        label.set_fontsize(const['fontsizeticks'])
    ...
    # Plot turning point
    line4 = ui_axes.plot(flowrate['interp_dt'][wt['index']], 
                        flowrate['interp_y_smooth'][wt['index']],
                        'o', markersize=10, 
    ...
    # Create legend
    lines = line1 + line2 + [line4[0]] + line7  # Combine all lines
    labels = [line.get_label() for line in lines]
    legend = ui_axes.legend(lines, labels, 
                          loc='southwest',
                          fontsize=const['fontsizelegend'])
    ...
    return {
        'primary_axis': ui_axes,
        'secondary_axis': ax2,
        'lines': [line1[0], line2[0], line3[0], line4[0], line7[0]],
        'legend': legend
    }
...
def update_model_down_plot_wendetangente():
    try:
        ...
        wt_result = wendetangente(
            {'interp_y_smooth': biogas_data.values, 'interp_dt': timestamps, 'sel_time': timestamps},
            {'interp_diff_substrate_feed': substrate_data.values},
        ...
        if wt_result is None:
            raise ValueError("Wendetangente estimation failed")

        plot_wendetangente_ui(0, ax1, 
