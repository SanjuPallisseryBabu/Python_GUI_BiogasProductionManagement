...
def feeding_plan_ui(start_date, simulation_results=None):
    """
    Calculate feeding plans based on simulation results
    ...
    """
    try:
        print("\n----- FEEDING PLAN DEBUGGING -----")
        print(f"Input start_date: {start_date}")
        print(f"Simulation results provided: {'Yes' if simulation_results is not None else 'No'}")
        ...
        if simulation_results is not None and 'time' in simulation_results:
            ...
            print(f"Using simulation timespan: {sim_duration_hours} hours = {days} days")
        else:
            # Default to 7 days if no simulation data
            days = 7
            print(f"DEFAULT: Using default timespan of {days} days")
            if simulation_results is not None:
                print(f"   Reason: 'time' key not found in simulation_results")
                print(f"   Available keys: {list(simulation_results.keys())}")
            
        # Generate time points
        time_daily = [start_date + timedelta(days=i) for i in range(days)]
        time_three_hourly = []
        
        # Generate time points every 3 hours
        for i in range(days):
            for hour in range(0, 24, 3):
                time_three_hourly.append(start_date + timedelta(days=i, hours=hour))
        
        # Generate sum values (in tons)
        # These would typically come from simulation, but we'll use plausible values
        if simulation_results is not None and 'substrate_feeding' in simulation_results:
            # Use simulation data if available
            print("Using actual simulation data for substrate feeding")
            substrate_daily = []
            for i in range(days):
                idx_start = i * 24 // 2  # Assuming 2-hour samples
                idx_end = (i+1) * 24 // 2
                if idx_end < len(simulation_results['substrate_feeding']):
                    daily_sum = np.sum(simulation_results['substrate_feeding'][idx_start:idx_end])
                    substrate_daily.append(daily_sum)
                else:
                    substrate_daily.append(0)
                    print(f"   Warning: Day {i+1} exceeds simulation data, using 0")
            
            sum_daily = np.array(substrate_daily)
            print(f"   Daily substrate values: {sum_daily}")
            
            # For three-hourly, split the daily values
            sum_three_hourly = []
            for daily_val in sum_daily:
                # Split the daily value into 8 periods (24 hours / 3 hours)
                three_hourly_vals = daily_val / 8 * np.ones(8)
                sum_three_hourly.extend(three_hourly_vals)
            
            sum_three_hourly = np.array(sum_three_hourly)
        else:
            # Use default values if no simulation data
            print("DEFAULT: Using default substrate feeding patterns")
            if simulation_results is not None:
                print(f"   Reason: 'substrate_feeding' key not found in simulation_results")
                print(f"   Available keys: {list(simulation_results.keys())}")
                
            base_daily = 20.0  # Base tonnage per day
            daily_pattern = np.array([0.9, 1.0, 1.1, 1.0, 0.95, 0.9, 0.85])  # Weekly pattern
            
            # Ensure we have enough values for the required days
            if len(daily_pattern) < days:
                daily_pattern = np.pad(daily_pattern, (0, days - len(daily_pattern)), 'wrap')
                print(f"   Extended daily pattern to {days} days")
            elif len(daily_pattern) > days:
                daily_pattern = daily_pattern[:days]
                print(f"   Truncated daily pattern to {days} days")
                
            sum_daily = base_daily * daily_pattern
            print(f"   Default daily values: {sum_daily}")
            
            # Create three-hourly pattern (8 periods per day)
            hourly_pattern = np.array([0.8, 0.6, 0.4, 0.8, 1.5, 1.8, 1.2, 0.9])  # Daily pattern
            sum_three_hourly = np.array([])
            
            for daily_val in sum_daily:
                # Normalize hourly pattern to sum to the daily value
                hourly_vals = hourly_pattern * daily_val / np.sum(hourly_pattern)
                sum_three_hourly = np.append(sum_three_hourly, hourly_vals)
            
            print(f"   Generated {len(sum_three_hourly)} three-hourly values")
        
        print("----- END FEEDING PLAN DEBUGGING -----\n")
...
def update_feeding_schedule():
    """
    Update the feeding schedule tables based on the current simulation results
    """
    try:
        print("\n----- UPDATE FEEDING SCHEDULE DEBUGGING -----")
        ...
        # Get start date from control tab or use current date
        start_date = None
        
        # Get UI values first
        try:
            ui_values = get_ui_values()
            print(f"UI values retrieved successfully")
            print(f"UI values keys: {list(ui_values.keys())}")
        except Exception as ui_error:
            print(f"ERROR getting UI values: {ui_error}")
            ui_values = {}
        
        # Try to get the start date from the control tab
        if hasattr(start_date_field, 'get_date'):
            start_date = start_date_field.get_date()
            print(f"Using start date from DateEntry field: {start_date}")
        elif 'simulation_results' in global_vars and 'start_date' in ui_values:
            start_date = ui_values['start_date']
            print(f"Using start date from ui_values: {start_date}")
        else:
            # Default to current date
            start_date = datetime.now().date()
            print(f"DEFAULT: Using current date as start date: {start_date}")
            print(f"   Reason: DateEntry field not accessible and/or 'start_date' not in ui_values")
            if 'simulation_results' in global_vars:
                print(f"   Note: simulation_results exists in global_vars")
            else:
                print(f"   Note: simulation_results does NOT exist in global_vars")
        
        # Get simulation results if available
        simulation_results = global_vars.get('simulation_results', None)
        if simulation_results is not None:
            print(f"Simulation results found in global_vars")
            print(f"Simulation results keys: {list(simulation_results.keys())}")
        else:
            print(f"DEFAULT: No simulation results found in global_vars")
            
        # Calculate feeding plan
        print("Calling feeding_plan_ui with start_date and simulation_results...")
        _, sum_three_hourly, time_three_hourly, _, sum_daily, time_daily = feeding_plan_ui(start_date, simulation_results)
        
        print(f"Received feeding plan results:")
        print(f"   Daily values length: {len(sum_daily)}")
        print(f"   Three-hourly values length: {len(sum_three_hourly)}")
        
        # Define feeding proportions
        anteil_GPS = 4/8     # 50% Grass Silage
        anteil_mais = 1.5/8  # 18.75% Corn
        anteil_mist = 2/8    # 25% Manure 
        anteil_reste = 0.5/8 # 6.25% Fodder residues
        ...
        print("Updating daily feeding schedule...")
        for i, (time, sum_val) in enumerate(zip(time_daily, sum_daily)):
            ...
            # Insert into treeview with 2 decimal precision
        ...
        print("Updating three-hourly feeding schedule...")
        ...
        print("Feeding schedule updated successfully")
        print("----- END UPDATE FEEDING SCHEDULE DEBUGGING -----\n")
