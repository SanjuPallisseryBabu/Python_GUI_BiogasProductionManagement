...
def update_model_down_plot_pt1():
    ...
    try:
        ...
        new_colors = {
            ...
            'annotation': '#CCCCCC'    # Gray for annotations
        }
        ...
        # Calculate parameters based on original data for PT1 model
        feed_max = substrate_data.max()
...
# Turning Tangent model

def wendetangente(flowrate, feed, ind_sprung, feed_max, feed_min, 
                  flowrate_max, flowrate_min, ind_wp, steigung_wp, step_dir):
    """
    Python implementation of the wendetangente (inflection tangent) method
    ...
    flow_times = np.array(flowrate.interp_dt)
    flow_values = np.array(flowrate.interp_y_smooth)
    feed_values = np.array(feed.interp_diff_substrate_feed)
    
    # Ensure indices are within bounds
    total_length = len(flow_times)
    ind_sprung = min(max(0, ind_sprung), total_length - 1)
    ind_wp = min(max(0, ind_wp), total_length - 1)
    ...
    wt['zeit_sprung'] = flow_times[wt['ind_sprung']]
    
    # Inflection tangent equation
    wt['index'] = ind_wp
    wt['steigung'] = steigung_wp
    wt['wert'] = flow_values[wt['index']]
    wt['offset'] = wt['wert'] - wt['steigung'] * wt['index']
    wt['t'] = np.arange(total_length)
    wt['gleichung'] = wt['steigung'] * wt['t'] + wt['offset']
    
    # Determine Tu and Tg indices
    if 'down' in step_dir:
        # Find Tu index where equation exceeds flowrate_max
    ...
    if 'up' in step_dir:
        # Find Tu index where equation falls below flowrate_min
    ...
    wt['ind_Tu'] = min(max(0, wt['ind_Tu']), total_length - 1)
    wt['ind_Tg'] = min(max(0, wt['ind_Tg']), total_length - 1)
    
    # Calculate time differences
    tu_time_diff = (flow_times[wt['ind_Tu']] - flow_times[wt['ind_sprung']]) / np.timedelta64(1, 's')
    tg_time_diff = (flow_times[wt['ind_Tg']] - flow_times[wt['ind_Tu']]) / np.timedelta64(1, 's')
    twp_time_diff = (flow_times[wt['index']] - flow_times[wt['ind_sprung']]) / np.timedelta64(1, 's')
    
    # Calculate Tu, Tg, and Twp
    wt['Tu'] = tu_time_diff
    wt['Tg'] = tg_time_diff
    wt['Twp'] = twp_time_diff
    ...
    # Transfer functions
    wt['G_Twp'] = wt['K'] * tf([1], [wt['T_Twp'], 1]) * tf([1], [wt['T_Twp'], 1])
    wt['G_Tu'] = wt['K'] * tf([1], [wt['T_Tu'], 1]) * tf([1], [wt['T_Tu'], 1])
    wt['G_Tg'] = wt['K'] * tf([1], [wt['T_Tg'], 1]) * tf([1], [wt['T_Tg'], 1])
    wt['G_Tmean'] = wt['K'] * tf([1], [wt['T_mean'], 1]) * tf([1], [wt['T_mean'], 1])
    
    # Continuous to discrete conversion
    wt['Ta'] = 120  # sampling time
    wt['test_time'] = np.arange(0, wt['Ta'] * total_length, wt['Ta'])
    
    wt['Gd_Twp'] = c2d(wt['G_Twp'], wt['Ta'])
    wt['Gd_Tu'] = c2d(wt['G_Tu'], wt['Ta'])
    wt['Gd_Tg'] = c2d(wt['G_Tg'], wt['Ta'])
    wt['Gd_Tmean'] = c2d(wt['G_Tmean'], wt['Ta'])
    
    # Calculate step responses
    if 'down' in step_dir:
        input_signal = feed_values - feed_max
        wt['yd_Twp'] = lsim(wt['Gd_Twp'], input_signal, wt['test_time'])[1] + flowrate_max
        wt['yd_Tu'] = lsim(wt['Gd_Tu'], input_signal, wt['test_time'])[1] + flowrate_max
        wt['yd_Tg'] = lsim(wt['Gd_Tg'], input_signal, wt['test_time'])[1] + flowrate_max
        wt['yd_Tmean'] = lsim(wt['Gd_Tmean'], input_signal, wt['test_time'])[1] + flowrate_max
    
    if 'up' in step_dir:
        input_signal = feed_values - feed_min
        wt['yd_Twp'] = lsim(wt['Gd_Twp'], input_signal, wt['test_time'])[1] + flowrate_min
        wt['yd_Tu'] = lsim(wt['Gd_Tu'], input_signal, wt['test_time'])[1] + flowrate_min
        wt['yd_Tg'] = lsim(wt['Gd_Tg'], input_signal, wt['test_time'])[1] + flowrate_min
        wt['yd_Tmean'] = lsim(wt['Gd_Tmean'], input_signal, wt['test_time'])[1] + flowrate_min
    
    # Tg provides best results
    wt['yd'] = wt['yd_Tg']
    
    return wt

def update_model_down_plot_Wendetangente():
    """Update the step down plot with Wendetangente model using enhanced styling"""
    try:
        # Check if we have preprocessed data
        if global_vars['biogas_segment_down'] is None or global_vars['substrate_segment_down'] is None:
            print("No preprocessed data available. Please preprocess data first.")
        ...
        # Get preprocessed data and ensure it's not empty
        biogas_data = global_vars['biogas_segment_down']['SmoothedValueNum']
        substrate_data = global_vars['substrate_segment_down']['FeedingRate']
        timestamps = global_vars['timestamps_down']

        if len(biogas_data) == 0 or len(substrate_data) == 0:
            print("Empty data arrays detected.")
            return

        # Convert to numpy arrays
        biogas_values = np.array(biogas_data)
        substrate_values = np.array(substrate_data)

        # Calculate limits
        feed_max = float(substrate_values.max())
        feed_min = float(substrate_values.min())
        flowrate_max = float(biogas_values.max())
        flowrate_min = float(biogas_values.min())
        
        # Calculate y-axis limits with padding
        y_min = flowrate_min - 0.05 * flowrate_max
        y_max = flowrate_max + 0.05 * flowrate_max
        y_min_right = feed_min - 0.05 * feed_max
        y_max_right = feed_max + 0.05 * feed_max
        
        # Find step index
        diff_substrate = np.diff(substrate_values)
        if len(diff_substrate) > 0:
            ind_sprung = int(np.argmax(np.abs(diff_substrate)))
        else:
            print("Not enough data points for step detection.")
            return
        
        # Find inflection point
        slopes = np.diff(biogas_values) / np.diff(np.arange(len(biogas_values)))
        if len(slopes) > 0:
            ind_wp = int(np.argmax(np.abs(slopes))) + 1
            steigung_wp = float(slopes[ind_wp - 1])
        else:
            print("Not enough data points for slope calculation.")
            return
        ...
        flowrate_struct = DataStruct()
        flowrate_struct.interp_dt = timestamps
        flowrate_struct.interp_y_smooth = biogas_values
        
        feed_struct = DataStruct()
        feed_struct.interp_diff_substrate_feed = substrate_values

        try:
            # Calculate Wendetangente model
            wt_down = wendetangente(
                flowrate=flowrate_struct,
                feed=feed_struct,
                ind_sprung=ind_sprung,
                feed_max=feed_max,
                feed_min=feed_min,
            ...
            print(f"Error in wendetangente calculation: {e}")
            import traceback
            traceback.print_exc()
            return

        if wt_down is not None:
            try:
                # Style primary axis
                ax1.grid(True)
                ax1.tick_params(labelsize=const['fontsizeticks'])
                ...
                line1 = ax1.plot(timestamps, biogas_data, 
                               ...
                               label='Gas production flow rate')
                
                # Plot Wendetangente model result
                model_output = np.array(wt_down['yd'])
                if len(model_output) != len(timestamps):
                    model_output = np.interp(np.arange(len(timestamps)), 
                                           np.arange(len(model_output)), 
                                           model_output)
                line2 = ax1.plot(timestamps, model_output,
                               color=newcolors[2],
                               linewidth=const['linewidth'],
                               label='Wendetangente model')
                
                # Plot inflection tangent line
                line3 = ax1.plot(timestamps, wt_down['gleichung'],
                               color=[color_float]*3,
                               linewidth=const['linewidth'],
                               label='Wendetangente')
                
                # Plot inflection point
                if 'wp_index' in wt_down:
                    ax1.plot(timestamps[wt_down['wp_index']], 
                            biogas_values[wt_down['wp_index']], 
                            'o',
                            markerfacecolor=[color_float]*3,
                            markeredgecolor=[color_float]*3,
                            markersize=10,
                            label='Wendepunkt')

                # Style secondary axis
                ax2.set_ylim([y_min_right, y_max_right])
                ax2.spines['right'].set_color(newcolors[1])
                ax2.tick_params(axis='y', colors=newcolors[1])
                
                # Plot substrate feeding rate
                line4 = ax2.plot(timestamps, substrate_data,
                               color=newcolors[1],
                               linewidth=const['linewidth'],
                               label='Substrate feeding rate')
                ...
                ax2.set_ylabel('Substrate feeding rate [t/3h]',
                             ...
                             fontname=const['font'],
                             fontsize=const['fontsize'])
                ax1.set_title('Wendetangente method',
                            fontname=const['font'],
                            fontsize=const['fontsize'])
                ...
                legend = ax1.legend(lines, labels,
                                  loc='lower left',
                                  prop={'family': const['font'],
                                       'size': const['fontsizelegend']})

                # Calculate metrics
                model_output_flat = model_output.flatten()
                actual_output_flat = biogas_values.flatten()
                
                if len(model_output_flat) == len(actual_output_flat):
                    metrics = calculate_metrics(actual_output_flat, model_output_flat)
                    update_metrics_table(tree_down, metrics)
        ...
        print(f"Error in update_model_down_plot_Wendetangente: {e}")
        import traceback
        traceback.print_exc()

def update_model_up_plot_Wendetangente():
    """Update the step up plot with Wendetangente model using enhanced styling"""
    try:
        # Check if we have preprocessed data
        if global_vars['biogas_segment_up'] is None or global_vars['substrate_segment_up'] is None:
            print("No preprocessed data available. Please preprocess data first.")
        ...
        # Find step index for upward step
        diff_substrate = np.diff(substrate_values)
        if len(diff_substrate) > 0:
            # For upward step, find maximum positive change
            ind_sprung = int(np.argmax(diff_substrate))
        else:
            print("Not enough data points for step detection.")
            return
        
        # Find inflection point for upward step
        slopes = np.diff(biogas_values) / np.diff(np.arange(len(biogas_values)))
        if len(slopes) > 0:
            # For upward step, find maximum positive slope
            ind_wp = int(np.argmax(slopes)) + 1
            steigung_wp = float(slopes[ind_wp - 1])
        else:
            print("Not enough data points for slope calculation.")
            return
        ...
        try:
            # Calculate Wendetangente model for upward step
            wt_up = wendetangente(
                flowrate=flowrate_struct,
                feed=feed_struct,
                ind_sprung=ind_sprung,
                ...
                ind_wp=ind_wp,
                steigung_wp=steigung_wp,
                step_dir='up'  # Changed to 'up' for upward step
            )
        except Exception as e:
            print(f"Error in wendetangente calculation: {e}")
            import traceback
        ...
        if wt_up is not None:
            try:
                ...
                # Plot Wendetangente model result
                model_output = np.array(wt_up['yd'])
                if len(model_output) != len(timestamps):
                    model_output = np.interp(np.arange(len(timestamps)), 
                                           np.arange(len(model_output)), 
                                           model_output)
                ...
                # Plot inflection tangent line
                line3 = ax1.plot(timestamps, wt_up['gleichung'],
                               color=[color_float]*3,
                               linewidth=const['linewidth'],
                               label='Wendetangente')
                
                # Plot inflection point
                if 'wp_index' in wt_up:
                    ax1.plot(timestamps[wt_up['wp_index']], 
                            biogas_values[wt_up['wp_index']], 
                            'o',
                            markerfacecolor=[color_float]*3,
                            markeredgecolor=[color_float]*3,
                            markersize=10,
                ...
                ax2.set_ylabel('Substrate feeding rate [t/3h]',
                             ...
                             fontname=const['font'],
                             fontsize=const['fontsize'])
                ax1.set_title('Wendetangente method (Step Up)',
                            fontname=const['font'],
                            fontsize=const['fontsize'])
        ...
        print(f"Error in update_model_up_plot_Wendetangente: {e}")
        import traceback
        traceback.print_exc()
