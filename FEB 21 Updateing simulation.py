...
# Example usage:
# result = calculate_zeitprozentkennwert(flowrate, feed, ind_sprung, feed_max, feed_min, 
#                                       flowrate_max, flowrate_min, timestamps, step_direction)
# if result:
#     # Get the transfer function coefficients
#     coeffs = result["transfer_function_coeffs"]
#     e, f, c, d = coeffs["e"], coeffs["f"], coeffs["c"], coeffs["d"]
#     print(f"Transfer function Gd(z) = ({e}z + {f})/(z² + {c}z + {d})")
...
import scipy.linalg
from scipy.signal import acker
...
result = calculate_zeitprozentkennwert(
    global_vars['biogas_segment_up']['SmoothedValueNum'].values,
    global_vars['substrate_segment_up']['FeedingRate'].values,
    np.argmax(np.diff(global_vars['substrate_segment_up']['FeedingRate'].values)),
    global_vars['substrate_segment_up']['FeedingRate'].max(),
    global_vars['substrate_segment_up']['FeedingRate'].min(),
    global_vars['biogas_segment_up']['SmoothedValueNum'].max(),
    global_vars['biogas_segment_up']['SmoothedValueNum'].min(),
    global_vars['timestamps_up'],
    step_direction="up"
)
...
D = np.array([[0]])  # Direct transmission term (usually zero)
...
# Define desired pole locations (modify based on system requirements)
desired_poles = np.array([0, 0])  # Placing poles at the origin for stability
# Compute Gain Matrix K using pole placement
K = acker(A, B, desired_poles)


# Assuming G_ss_PT2 is an object or dictionary containing A, B, and C matrices
I = np.eye(len(K))  # Identity matrix
M = I - A + B @ K  # Matrix operation

# Compute Pre-filter F
F = np.linalg.inv(C @ np.linalg.inv(M) @ B)

# F = np.linalg.inv(C @ np.linalg.inv(B @ K - A) @ B)
...
def simulate_feedback_system():
    """Run the simulation using the computed A, B, C matrices."""
    for k in range(time_steps - 1):
        delta_y_soll = r[k] - y[k]  # Compute the error
        delta_yd_soll = delta_y_soll  # Define the target change in y

        # Compute Control Signal
        t_ss_PT2_u = F * delta_yd_soll
        control_signal = t_ss_PT2_u - K @ x[:, k]

        # Apply State-Space Model Equations
        x[:, k+1] = A @ x[:, k] + B @ control_signal  # State update
        y[k] = C @ x[:, k]  # Compute output
...
def plot_regler_simulink_anti_windup_ui(i, ui_axes, start_date, out, t_stop, Ta):
    ...
    time_axis = np.linspace(start_date, start_date + t_stop, len(out['mit_antiwindup_y_modif']))

    # Left Y-axis: Biogas Production Rate
    fig, ax1 = plt.subplots()
    ax1.set_xlabel('Time')
    ax1.set_ylabel('Gas production flow rate [m³/h]', color='tab:blue')
    ax1.plot(time_axis, out['mit_antiwindup_y_modif'], label='Simulated Gas Production Rate', color='tab:blue', linewidth=1.5)
    ax1.plot(time_axis, out['y_soll'], '--', label='Setpoint', color='tab:green', linewidth=1.5, alpha=0.8)
    ax1.tick_params(axis='y', labelcolor='tab:blue')
    ax1.legend(loc='upper left')

    # Right Y-axis: Substrate Feeding Rate
    ax2 = ax1.twinx()
    ax2.set_ylabel('Substrate feeding rate [t/2h]' if Ta == 2 else 'Substrate feeding rate [t/3h]', color='tab:red')
    ax2.plot(time_axis, out['mit_antiwindup_u_modif'], label='Substrate Feeding Rate', color='tab:red', linewidth=1.5)
    ax2.tick_params(axis='y', labelcolor='tab:red')
    ax2.legend(loc='upper right')

    plt.title('Discrete PI-Controller (+ Root Locus) with Anti-Windup')
    plt.grid()
    plt.show()

# === Start Simulation ===
def start_simulation():
    global y_out, u_out, x_out, out
    y_out, u_out, x_out = simulate_feedback_system()
    out = {
        'mit_antiwindup_y_modif': y_out,
        'y_soll': r,
        'mit_antiwindup_u_modif': u_out
    }
    plot_regler_simulink_anti_windup_ui(1, None, 0, out, time_steps, 2)
