...
hourly_frame = ttk.LabelFrame(feeding_tab, text="Hourly feeding schedule", padding=10)
...
def feeding_plan_ui(start_date, simulation_results=None):
    ...
    Returns:
    - Tuple of (None, sum_three_hourly, time_three_hourly, None, sum_daily, time_daily)
    """
    try:
        ...
        # Create datetime objects for daily and three-hourly schedules
        ...
        # Generate time points every 3 hours (8 periods per day) with 30 minutes offset to match MATLAB
        for i in range(days):
            for hour in range(0, 24, 3):
                # Add 30 minutes to match MATLAB format with :30:00
                time_three_hourly.append(start_date + timedelta(days=i, hours=hour, minutes=30))
        ...
        if simulation_results is not None and 'substrate_feeding' in simulation_results:
            ...
            # Create three-hourly values directly from simulation data
            sum_three_hourly = []
            
            # Process each day
            for day_idx, daily_val in enumerate(sum_daily):
                ...
                if day_start < len(simulation_results['substrate_feeding']):
                    ...
                    # FIXED: Better algorithm to create 8 three-hour periods from 12 two-hour samples
                    # For each day we have 24 hours, with data every 2 hours (12 samples)
                    # We need to create 8 three-hour blocks (each representing 3 hours)
                    
                    # Check if we have enough data points
                    if len(day_values) < 4:  # Need at least 4 points to interpolate reasonably
                        print(f"ERROR: Insufficient data for day {day_idx+1}, only {len(day_values)} points")
                        return (None, np.array(sum_three_hourly), time_three_hourly[:len(sum_three_hourly)], 
                                None, sum_daily[:day_idx], time_daily[:day_idx])
                    
                    # Create 8 three-hour values from however many two-hour values we have
                    three_hour_values = []
                    
                    # If we have 12 values (complete day of 2-hour samples)
                    if len(day_values) >= 12:
                        # Strategy: Each 3-hour period corresponds to 1.5 two-hour samples
                        # We'll distribute the 12 two-hour samples across 8 three-hour periods
                        
                        # Period 1: 1.5 two-hour samples (0, 1, half of 2)
                        three_hour_values.append(day_values[0] + day_values[1] + 0.5 * day_values[2])
                        
                        # Period 2: 1.5 two-hour samples (half of 2, 3, half of 4)
                        three_hour_values.append(0.5 * day_values[2] + day_values[3] + 0.5 * day_values[4])
                        
                        # Period 3: 1.5 two-hour samples (half of 4, 5, half of 6)
                        three_hour_values.append(0.5 * day_values[4] + day_values[5] + 0.5 * day_values[6])
                        
                        # Period 4: 1.5 two-hour samples (half of 6, 7, half of 8)
                        three_hour_values.append(0.5 * day_values[6] + day_values[7] + 0.5 * day_values[8])
                        
                        # Period 5: 1.5 two-hour samples (half of 8, 9, half of 10)
                        three_hour_values.append(0.5 * day_values[8] + day_values[9] + 0.5 * day_values[10])
                        
                        # Period 6: 1.5 two-hour samples (half of 10, 11, small portion of next day if available)
                        if day_idx + 1 < len(sum_daily) and (day_start + 12) < len(simulation_results['substrate_feeding']):
                            next_day_value = simulation_results['substrate_feeding'][day_start + 12]
                            three_hour_values.append(0.5 * day_values[10] + day_values[11])
                        else:
                            three_hour_values.append(0.5 * day_values[10] + day_values[11])
                        
                        # For periods 7 and 8, distribute remaining value
                        remaining_total = daily_val - sum(three_hour_values)
                        if remaining_total > 0:
                            # Split remaining between periods 7 and 8
                            three_hour_values.append(remaining_total * 0.5)
                            three_hour_values.append(remaining_total * 0.5)
                        else:
                            # Use last value as reference
                            last_val = three_hour_values[-1]
                            three_hour_values.append(last_val * 0.9)
                            three_hour_values.append(last_val * 0.9)
                    else:
                        # If we have fewer points, distribute evenly
                        print(f"WARNING: Only {len(day_values)} data points for day {day_idx+1}, distributing evenly")
                        
                        # Calculate how much each existing value contributes to 8 output values
                        samples_per_value = 8.0 / len(day_values)
                        
                        # Create intermediate values by simple interpolation
                        for i in range(8):
                            # Find position in original array
                            pos = i / samples_per_value
                            idx_low = int(pos)
                            idx_high = min(idx_low + 1, len(day_values) - 1)
                            fraction = pos - idx_low
                            
                            # Linear interpolation between adjacent values
                            value = (1 - fraction) * day_values[idx_low]
                            if idx_low != idx_high:
                                value += fraction * day_values[idx_high]
                                
                            three_hour_values.append(value)
                    
                    # Normalize to ensure the three-hourly values sum to the daily total
                    current_sum = sum(three_hour_values)
                    if current_sum > 0:  # Avoid division by zero
                        scale_factor = daily_val / current_sum
                        three_hour_values = [val * scale_factor for val in three_hour_values]
                    
                    # Add values to the array
                    sum_three_hourly.extend(three_hour_values)
                else:
        ...
        if len(sum_daily) < days:
            ...
            time_three_hourly = time_three_hourly[:days*8]
        ...
        if len(sum_three_hourly) < days * 8:
            print(f"ERROR: Insufficient three-hourly values. Expected {days*8}, got {len(sum_three_hourly)}")
            # Truncate time array to match available data
            time_three_hourly = time_three_hourly[:len(sum_three_hourly)]
            
        elif len(sum_three_hourly) > days * 8:
            print(f"WARNING: Too many three-hourly values. Expected {days*8}, got {len(sum_three_hourly)}")
            # Truncate to requested number
            sum_three_hourly = sum_three_hourly[:days*8]
            
        # Print summary for debugging
        print(f"Generated {len(time_daily)} daily values and {len(time_three_hourly)} three-hourly values")
        print(f"Daily sums: {sum_daily}")
        print(f"Three-hourly sample: {sum_three_hourly[:8]}")
            
        return (None, sum_three_hourly, time_three_hourly, None, sum_daily, time_daily)
...
def update_feeding_schedule():
    ...
    try:
        ...
        _, sum_three_hourly, time_three_hourly, _, sum_daily, time_daily = result
        
        # Check if data was returned
        if len(sum_daily) == 0 or len(sum_three_hourly) == 0:
        ...
        for i, (time, sum_val) in enumerate(zip(time_daily, sum_daily)):
            # Format date for display to match MATLAB format: "15-Nov-2024"
        ...
        # Update three-hourly feeding schedule
        print("\nFirst few entries of three-hourly feeding schedule:")
        max_debug_entries = min(8, len(time_three_hourly))  # Print first 8 entries or fewer
        
        for i, (time, sum_val) in enumerate(zip(time_three_hourly, sum_three_hourly)):
            # Format datetime to match MATLAB time format
...
def export_to_excel():
    ...
    try:
        ...
        hourly_sheet = wb.create_sheet("Hourly Feeding Schedule")
