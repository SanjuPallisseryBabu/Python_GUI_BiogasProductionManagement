...
# Function to create and display the downward step plot
def create_plot_step_downward():
    if biogas_data is not None and substrate_data is not None:
        ...
        try:
            ...
            # Set the title for the downward plot
            ax1.set_title('Plot Data Step Downward', fontsize=20, fontweight='bold', fontname='Arial')

            # Second y-axis for substrate feeding
            ax2 = ax1.twinx()
            ax2.step(filtered_substrate['TimeStamp'], filtered_substrate['ValueNum'][::-1], 'r-', label='Substrate feeding (Step Downward)')
...
window = tk.Tk()
window.title("Biogas Production and Substrate Feeding - Step Downward")
...
# Button to plot the downward step graph
plot_button = ttk.Button(frame, text="Plot Data Step Downward", state='disabled', command=create_plot_step_downward)
