...
#File Time constant sum @JAN06 Thesis main work03
##################################################
...
style.configure('TButton', background='#E6E6E6', font=('Arial', 10))

# Configure grid weights for `control_tab`
control_tab.grid_columnconfigure(0, weight=3)  # Feedback section
control_tab.grid_columnconfigure(1, weight=1)  # Saturation section
control_tab.grid_rowconfigure(1, weight=1)     # Plot area

# Feedback Controller Section Frame
feedback_frame = ttk.Frame(control_tab)
feedback_frame.grid(row=0, column=0, sticky="nsew", padx=(15, 10), pady=(10, 10))

# Saturations Section Frame
saturation_frame = ttk.Frame(control_tab)
saturation_frame.grid(row=0, column=1, sticky="nsew", padx=(10, 15), pady=(10, 10))

# Feedback Controller Section
ttk.Label(feedback_frame, text="Feedback Controller", font=('Arial', 12, 'bold')).grid(
    row=0, column=0, columnspan=6, pady=(10, 15)
)

# Gas production flow rate (initial state)
ttk.Label(feedback_frame, text="Gas production flow rate, initial state").grid(
    row=1, column=0, sticky="e", padx=(6, 3), pady=(6, 3)
)

# Create a small sub-frame to hold Entry + [m³/h] label tight together
gas_prod_init_frame = ttk.Frame(feedback_frame)
gas_prod_init_frame.grid(row=1, column=1, columnspan=2, sticky="w", padx=(3,0), pady=(6,3))

gas_prod_initial = ttk.Entry(gas_prod_init_frame, width=15)
gas_prod_initial.grid(row=0, column=0, sticky="w", padx=(0,0))

# The [m³/h] label, placed right next to the Entry with minimal padding
ttk.Label(gas_prod_init_frame, text="[m³/h]").grid(
...
ttk.Label(feedback_frame, text="Gas production flow rate, setpoint").grid(
    row=1, column=3, sticky="e", padx=(6, 3), pady=(6, 3)
)
gas_prod_setpt_frame = ttk.Frame(feedback_frame)
gas_prod_setpt_frame.grid(row=1, column=4, columnspan=2, sticky="w", padx=(3,0), pady=(6,3))

gas_prod_setpoint = ttk.Entry(gas_prod_setpt_frame, width=15)
gas_prod_setpoint.grid(row=0, column=0, sticky="w", padx=(0,0))

ttk.Label(gas_prod_setpt_frame, text="[m³/h]").grid(
    row=0, column=1, sticky="w", padx=(2,0)
)

# -----------------------------------------------------------

# Substrate feeding rate (initial state)
substrate_frame = ttk.Frame(feedback_frame)
substrate_frame.grid(row=2, column=0, columnspan=6, pady=(10, 5), sticky="n")
substrate_frame.grid_columnconfigure(0, weight=1)
substrate_frame.grid_columnconfigure(1, weight=1)
substrate_frame.grid_columnconfigure(2, weight=1)

ttk.Label(substrate_frame, text="Substrate feeding rate, initial state").grid(
    row=0, column=0, sticky="e", padx=(6, 3), pady=(6, 3)
)
substrate_feeding_initial = ttk.Entry(substrate_frame, width=15)
substrate_feeding_initial.grid(row=0, column=1, sticky="w", padx=(3, 2), pady=(6, 3))
ttk.Label(substrate_frame, text="[t/2h]").grid(
    row=0, column=2, sticky="w", padx=(2, 2), pady=(6, 3)  # Reduced horizontal padding
)

# Start date
ttk.Label(feedback_frame, text="Start date").grid(
    row=3, column=0, sticky="e", padx=(6, 3), pady=(6, 3)
)
# Replace ttk.Entry with tkcalendar's DateEntry
start_date_control = DateEntry(
    feedback_frame, 
    width=15,
    date_pattern='mm/dd/yyyy',  # This ensures the mm/dd/yyyy format
    background='white',         # Customize colors as needed
    foreground='black', 
    borderwidth=2
)
start_date_control.grid(row=3, column=1, sticky="w", padx=(3, 2), pady=(6, 3))

# -----------------------------------------------------------
# Simulation duration row - refactor similarly
ttk.Label(feedback_frame, text="Simulation duration").grid(
    row=3, column=3, sticky="e", padx=(6, 3), pady=(6, 3)
)

# Sub-frame for Entry + [days] label
duration_frame = ttk.Frame(feedback_frame)
duration_frame.grid(row=3, column=4, columnspan=2, sticky="w", padx=(3,0), pady=(6,3))

simulation_duration = ttk.Entry(duration_frame, width=15)
simulation_duration.grid(row=0, column=0, sticky="w", padx=(0,0))

# The [days] label, placed close to the Entry
ttk.Label(duration_frame, text="[days]").grid(
    row=0, column=1, sticky="w", padx=(2,0)
)

# Start simulation button
start_button_frame = ttk.Frame(feedback_frame)
start_button_frame.grid(row=4, column=0, columnspan=6, pady=(20, 0), sticky="n")
start_button_frame.grid_columnconfigure(0, weight=1)

start_button = ttk.Button(start_button_frame, text="start simulation")
start_button.grid(row=0, column=0, padx=(10, 10))

# Saturation Section
ttk.Label(saturation_frame, text="Saturations", font=('Arial', 12, 'bold')).grid(
    row=0, column=0, columnspan=3, pady=(10, 15)
)

# Substrate feeding rate (max)
ttk.Label(saturation_frame, text="Substrate feeding rate (max)").grid(
    row=1, column=0, sticky="e", padx=(6, 3), pady=(6, 3)
)

# Frame to hold Entry + [t/2h]
max_substrate_frame = ttk.Frame(saturation_frame)
max_substrate_frame.grid(row=1, column=1, columnspan=2, sticky="w", padx=(3, 0), pady=(6, 3))

substrate_feeding_max = ttk.Entry(max_substrate_frame, width=15)
substrate_feeding_max.grid(row=0, column=0, sticky="w", padx=(0, 0))

ttk.Label(max_substrate_frame, text="[t/2h]").grid(
    row=0, column=1, sticky="w", padx=(2, 0)
)

# Substrate feeding rate (min)
ttk.Label(saturation_frame, text="Substrate feeding rate (min)").grid(
    row=2, column=0, sticky="e", padx=(6, 3), pady=(10, 3)
)

# Another frame for Entry + [t/2h]
min_substrate_frame = ttk.Frame(saturation_frame)
min_substrate_frame.grid(row=2, column=1, columnspan=2, sticky="w", padx=(3, 0), pady=(10, 3))

substrate_feeding_min = ttk.Entry(min_substrate_frame, width=15)
substrate_feeding_min.grid(row=0, column=0, sticky="w", padx=(0, 0))

ttk.Label(min_substrate_frame, text="[t/2h]").grid(
    row=0, column=1, sticky="w", padx=(2, 0)
)

# Plot Section
plot_frame = ttk.Frame(control_tab)
plot_frame.grid(
...
fig_control = Figure(figsize=(6, 3), dpi=100, facecolor='white')
plot_canvas_control = FigureCanvasTkAgg(fig_control, master=plot_frame)
plot_canvas_control.get_tk_widget().grid(row=0, column=0, sticky="nsew", padx=(5, 5), pady=(10, 10))
...
feedback_frame.grid_columnconfigure(5, weight=1)

saturation_frame.grid_columnconfigure(0, weight=1)
saturation_frame.grid_columnconfigure(1, weight=1)
saturation_frame.grid_columnconfigure(2, weight=1)
