...
def run_simulation():
    ...
    try:
        ...
        setpoint = ui_values["gas_flow_setpoint"] - ui_values["gas_flow_initial"]
        ...
        for k in range(1, samples):
            # Prefilter (f_ss_PT2'u)
            prefilter_output = F * setpoint
            
            # State feedback (K_ss_PT2'vec)
            state_vector = x[k-1, :].reshape(-1, 1)
            # Fix array to scalar conversion by explicitly extracting the value
            state_feedback = float((K @ state_vector)[0, 0])
            
            # Control signal calculation (summation node)
            u_unbounded[k] = prefilter_output - state_feedback
            
            # Saturation block (nonlinear element)
            u_bounded[k] = np.clip(u_unbounded[k], feed_min, feed_max)
            substrate_feeding[k] = u_bounded[k]
            
            # Input to state-space model (G_ss_PT2_B'u)
            # Handle B*u properly based on dimensions
            if B.ndim == 2 and B.shape[1] == 1:  # B is a column vector
                Bu = B * u_bounded[k]  
            else:
                Bu = B @ np.array([[u_bounded[k]]])  # Matrix multiplication
            
            # State update equation (integrator and A feedback)
            x[k, :] = (A @ state_vector + Bu).flatten()
            
            # Output equation (G_ss_PT2_C'vec)
            # Fix array to scalar conversion by explicitly extracting the value
            biogas[k] = float((C @ x[k, :].reshape(-1, 1))[0, 0])
