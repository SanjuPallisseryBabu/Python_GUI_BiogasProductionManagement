...
def update_model_up_plot():
    ...
    try:
        ...
        if zk_up is not None:
            # Store the transfer function coefficients in global_vars
            if 'transfer_function_coeffs' in zk_up:
                if 'control_system_params' not in global_vars:
                    global_vars['control_system_params'] = {}
                
                global_vars['control_system_params']['transfer_function_coeffs'] = zk_up['transfer_function_coeffs']
                print("Stored transfer function coefficients:")
                print(global_vars['control_system_params']['transfer_function_coeffs'])
            else:
                print("Warning: No transfer function coefficients in zk_up")
            ...
            global_vars["up_scenario_data"] = {
                ...
                "flowrate_max": flowrate_max,
                "flowrate_min": flowrate_min,
                # Transfer function coefficients (added for control system)
                "transfer_function_coeffs": zk_up.get('transfer_function_coeffs', {})
            }
...
def select_model():
    ...
    try:
        ...
        # Store the upward response data
        if "up_scenario_data" in global_vars:
            ...
            if all(param in global_vars["up_scenario_data"] for param in required_params):
                ...
                # Initialize control_system_params if it doesn't exist
                if 'control_system_params' not in global_vars:
                    global_vars['control_system_params'] = {}
                
                # Copy all data from up_scenario_data to control_system_params
                global_vars['control_system_params'] = global_vars["up_scenario_data"]
                
                # Check if transfer_function_coeffs is included
                if 'transfer_function_coeffs' in global_vars["up_scenario_data"]:
                    print("Transfer function coefficients found in up_scenario_data. Using those.")
                else:
                    print("WARNING: No transfer function coefficients found. Using default values.")
                    # Add default transfer function coefficients as a fallback
                    global_vars['control_system_params']['transfer_function_coeffs'] = {
                        'e': 0.01,
                        'f': 0.009,
                        'c': 1.8,
                        'd': 0.82
                ...
                # Print the stored transfer function coefficients for debugging
                if 'transfer_function_coeffs' in global_vars['control_system_params']:
                    print("Transfer function coefficients stored in control_system_params:")
                    print(global_vars['control_system_params']['transfer_function_coeffs'])
