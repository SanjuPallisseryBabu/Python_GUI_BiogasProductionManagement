...
def resample_transfer_function():
    ...
    using discrete-to-discrete (d2d) conversion.
    """
    try:
        # Get the sampling time
        Ts_2h = 7200  # 2 hours in seconds
        
        # Get the original transfer function coefficients
        if 'pt2_Gd_num' not in global_vars or 'pt2_Gd_den' not in global_vars:
            print("Error: Transfer function coefficients not found. Run store_transfer_function_coefficients first.")
            return None
        
        # Get the original discrete transfer function coefficients
        pt2_Gd_num = global_vars['pt2_Gd_num']
        pt2_Gd_den = global_vars['pt2_Gd_den']
        
        # Get original sampling time from the model (assuming it's stored in global_vars)
        original_Ts = 120  # Example value, replace with actual source
        if 'original_Ts' in global_vars:
            original_Ts = global_vars['original_Ts']
        
        # Create a discrete transfer function object
        original_system = signal.dlti(pt2_Gd_num, pt2_Gd_den, dt=original_Ts)
        
        # Convert the system to a continuous-time transfer function
        cont_system = signal.dlti(*signal.cont2discrete(
            (original_system.num, original_system.den), 
            dt=original_Ts, 
            method='zoh'
        ), dt=None)
        
        # Convert the continuous system back to discrete with the new sampling time
        G_2h = signal.dlti(*signal.cont2discrete(
            (cont_system.num, cont_system.den), 
            dt=Ts_2h, 
            method='zoh'
        ), dt=Ts_2h)
        
        # Store the resampled transfer function for later use
        global_vars['G_2h_num'] = G_2h.num
        global_vars['G_2h_den'] = G_2h.den
        global_vars['G_2h'] = G_2h
        
        print("Transfer function resampled to 2-hour sampling time successfully.")
        print(f"Resampled numerator: {G_2h.num}")
        print(f"Resampled denominator: {G_2h.den}")
        
        return G_2h
        
    except Exception as e:
        print(f"Error resampling transfer function: {e}")
        traceback.print_exc()
        return None
...
def design_deadbeat_controller():
    ...
    try:
        ...
        if 'A' not in global_vars or 'B' not in global_vars:
            print("Error: State-space matrices not found. Run convert_to_state_space first.")
            return None
        ...
        # Set desired poles at the origin [0, 0] for deadbeat control
        desired_poles = [0, 0]  # Origin for deadbeat control
...
def calculate_prefilter_gain():
    """
    Calculate a prefilter gain to ensure the closed-loop system has unity DC gain.
    """
    try:
        # Check if required variables exist
        if ('A' not in global_vars or 'B' not in global_vars or 
            'C' not in global_vars or 'K' not in global_vars):
            print("Error: Required matrices not found. Run previous steps first.")
            return None
        ...
        # Calculate (I - A + B*K)
        I_minus_A_plus_BK = I - A + B @ K
        
        # Calculate inverse
        inverse_term = np.linalg.inv(I_minus_A_plus_BK)
        
        # Calculate prefilter gain (similar to MATLAB formula)
        f_ss_PT2 = 1.0 / (C @ inverse_term @ B)
        ...
        print("Warning: Matrix inversion encountered issues. Using alternate calculation.")
        try:
            A = global_vars['A']
            B = global_vars['B']
            C = global_vars['C']
            ...
            # Create closed-loop system matrices
            A_cl = A - B @ K
            
            # Calculate DC gain as C * (I-A_cl)^(-1) * B
            I = np.eye(A_cl.shape[0])
            dc_gain = C @ np.linalg.inv(I - A_cl) @ B
            
            # Prefilter is the inverse of the DC gain
            f_ss_PT2 = 1.0 / dc_gain
            
            global_vars['f_ss_PT2'] = f_ss_PT2
            print("Prefilter gain calculated using alternate method.")
            print(f"Prefilter gain f_ss_PT2: {f_ss_PT2}")
            
            return f_ss_PT2
            
        except Exception as e:
            print(f"Error in alternate prefilter calculation: {e}")
            traceback.print_exc()
            return None
...
def initialize_control_system():
    ...
    try:
        ...
        # Extract transfer function coefficients directly
        # We already have the coefficients e, f, c, d from the calculations above
        
        # Store these as the transfer function coefficients (similar to MATLAB's num and den)
        pt2_Gd_num = np.array([e, f])  # Numerator coefficients 
        pt2_Gd_den = np.array([1, -c, -d])  # Denominator coefficients with leading 1
        
        # Instead of using tf2ss, manually create state-space matrices
        ...
        G_ss_PT2 = ctrl.StateSpace(A, B, C, D)
        
        # Place poles exactly at desired locations
        ...
        print(f"Transfer Function Numerator: {pt2_Gd_num}")
        print(f"Transfer Function Denominator: {pt2_Gd_den}")
        print(f"State-Space A matrix:\n{G_ss_PT2.A}")
        print(f"State-Space B matrix:\n{G_ss_PT2.B}")
        print(f"State-Space C matrix:\n{G_ss_PT2.C}")
        print(f"State-Space D matrix:\n{G_ss_PT2.D}")
        ...
        return {
            ...
            'pt2_Gd_num': pt2_Gd_num,  # Store numerator coefficients
            'pt2_Gd_den': pt2_Gd_den   # Store denominator coefficients
        }
